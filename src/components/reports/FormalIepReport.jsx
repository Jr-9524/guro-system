...
const valueOrMissing = (value) => {
  ...
  if (Array.isArray(value)) {
    const items = value.flat().filter(Boolean).map(String);
    return items.length ? items.join(", ") : missing;
  }
  if (typeof value === "object") {
    const items = Object.values(value).flat().filter(Boolean).map(String);
    return items.length ? items.join(", ") : missing;
  }
  return String(value).trim() || missing;
};

const formatOptionalDate = (value) => (value ? formatDate(value) : missing);
...
const ReportField = ({ label, value, wide = false }) => (
  <div className={`iep-report-field ${wide ? "iep-report-field-wide" : ""}`}>
    <dt>{label}</dt>
  ...
  items?.length ? (
    <ul className="iep-report-list">
      {items.map((item, index) => (
        <li key={`${String(item)}-${index}`}>{valueOrMissing(item)}</li>
      ))}
    </ul>
  ) : (
...
const FormalIepReport = ({
  ...
  const learnerName = learner.fullName || missing;
  const preparedBy =
    details.preparedBy ||
    progressPlan.responsible ||
    data.preparedBy ||
    iep?.createdBy ||
    missing;
  ...
  return (
    <article
      className="iep-report-document"
      aria-label="Individualized Education Program"
    >
      <header className="iep-report-header">
        <p className="iep-report-system">GURO SYSTEM</p>
        <p className="iep-report-school">{valueOrMissing(learner.school)}</p>
        ...
        <div className="iep-report-meta">
          ...
          <span>
            ...
            {valueOrMissing(iep?.status)}
          </span>
          <span>
            <strong>Completion</strong>
            {getCompletionPercent(iep)}%
          </span>
          <span>
            <strong>Created</strong>
            {formatOptionalDate(iep?.createdAt)}
          </span>
          <span>
            <strong>Last modified</strong>
            {formatOptionalDate(iep?.lastModified)}
          </span>
          <span>
            <strong>IEP period</strong>
            {formatOptionalDate(dates.startDate)} to{" "}
            {formatOptionalDate(dates.endDate)}
          </span>
        </div>
      ...
      <ReportSection number="I" title="Learner Information">
        <dl className="iep-report-grid">
          ...
          <ReportField label="Grade level" value={learner.gradeLevel} />
          <ReportField label="Section" value={student?.section} />
          <ReportField label="Age" value={learner.age} />
          <ReportField
            label="Communication mode"
            value={learner.communicationMode}
          />
          <ReportField
            label="Disability category"
            value={learner.disabilityCategory}
          />
          <ReportField
            label="Learning needs"
            value={learner.learningNeeds || data.learningNeeds}
          />
          <ReportField
            label="Support intensity"
            value={learner.supportIntensity}
          />
          <ReportField
            label="Developmental level"
            value={learner.developmentalLevel}
          />

          <ReportField
            label="Primary area of focus"
            value={info.primaryAreaOfFocus}
          />
        </dl>
      </ReportSection>
      ...
      <ReportSection number="II" title="Parent / Guardian Information">
        <dl className="iep-report-grid">
          <ReportField label="Name" value={learner.guardianName} />
          <ReportField
            label="Relationship"
            value={learner.guardianRelationship}
          />
          <ReportField label="Contact number" value={learner.guardianContact} />
          <ReportField label="Email" value={learner.guardianEmail} />
        </dl>
      </ReportSection>

      <ReportSection
        number="III"
        title="Present Level of Academic and Functional Performance"
      >
        <dl className="iep-report-grid">
        ...
        <ReportNarrative
          label="Needs"
          value={plaafp.challenges || plaafp.needs}
        />
        <ReportNarrative
          label="Assessment results"
          value={plaafp.assessmentResults}
        />
        <ReportNarrative
          label="Teacher observations"
          value={plaafp.teacherObservations}
        />
        <ReportNarrative label="Classroom impact" value={plaafp.impact} />
        <ReportNarrative
          label="PLAAFP statement"
          value={plaafp.aiDraft || plaafp.statement}
        />
      </ReportSection>
          ...
          goals.map((goal, index) => (
            <div className="iep-report-goal" key={goal.id || index}>
              <div className="iep-report-goal-heading">
                <h3>
                  Goal {index + 1}: {goal.area || "Area not specified"}
                </h3>
                <span>
                  {goal.status} / {goal.progressPercentage}%
                </span>
              </div>
              <p className="iep-report-goal-statement">
      ...
      <ReportSection number="V" title="Accommodations and Modifications">
        <ReportColumns
          groups={[
            ...
            [
              "Other Supports",
              [...toList(accommodations.other), ...toList(data.modifications)],
            ],
          ]}
        />
      </ReportSection>
      ...
      <ReportSection number="VI" title="Services and Supports">
              ...
              <tbody>
                {services.map((service, index) => (
                  <tr key={service.id || index}>
                    ...
                    <td>
                      {valueOrMissing(service.setting || service.location)}
                    </td>
        ...
        <ReportList
          items={[...new Set(goals.flatMap((goal) => goal.supports || []))]}
        />
      </ReportSection>
      ...
      <ReportSection number="VII" title="Progress Monitoring Plan">
        <dl className="iep-report-grid">
          <ReportField
            ...
            value={progressPlan.dataMethod}
          />
          <ReportField
            label="Collection frequency"
            value={progressPlan.frequency}
          />
          <ReportField
            label="Reporting schedule"
            value={progressPlan.parentReport}
          />
          <ReportField
            label="Person responsible"
            value={progressPlan.responsible}
          />
        </dl>
        <ReportNarrative
          label="Additional monitoring notes"
          value={progressPlan.notes}
        />
      </ReportSection>
      ...
      <ReportSection number="VIII" title="Recent Progress Notes">
        {notes.length ? (
            ...
            {notes.slice(0, visibleNoteCount).map((note) => {
              ...
              return (
                <div key={note.id}>
                  ...
                  {note.total > 0 && (
                    <small>
                      Recorded score: {note.score}/{note.total} (
                      {Math.round((note.score / note.total) * 100)}%)
                    </small>
                  )}
                </div>
              );
          ...
          <p className="iep-report-missing">
            No progress notes have been recorded yet.
          </p>
        )}
        {visibleNoteCount < notes.length && (
          <button
            type="button"
            className="report-screen-only btn btn-ghost btn-sm mt-3"
            onClick={() => setVisibleNoteCount((count) => count + 5)}
          >
      ...
      <ReportSection
        number={aiProgressSummary ? "X" : "IX"}
        title="Document Preparation and Signatures"
        breakBefore
      >
        <dl className="iep-report-grid iep-report-grid-three">
          <ReportField label="Prepared by" value={preparedBy} />
          <ReportField
            label="Review date"
            value={formatOptionalDate(dates.reviewDate)}
          />
          <ReportField
      ...
      <footer className="iep-report-footer">
        Generated by GURO System. Review all information before finalizing this
        document.
      </footer>
    </article>
  );
